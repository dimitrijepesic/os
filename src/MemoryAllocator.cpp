...
// >>> PORAVNAT HEADER (ceo broj blokova) <<<
...
void MemoryAllocator::init() {
    ...
    // ukupni blokovi u HEAP-u (uključujući header prostora)
...
int MemoryAllocator::mem_free(void* addr) {
    ...
    // kreći spajanje od levog suseda ako postoji (češće nastaje kontinuitet)
...
void MemoryAllocator::split(memBlock* blk, size_t needBlocks) {
    ...
    if (remainingPayloadBlocks >= HEADER_IN_BLOCKS + 1) {
        ...
        // desni header ide tačno posle payload-a levog (koristi poravnati headerUser!)
...
// agresivno spajanje ulevo/udesno sve dok postoji kontinuitet
void MemoryAllocator::tryCoalesce(memBlock* blk) {
    ...
    while (merged) {
        ...
        // spajanje sa desnim susedom: [blk(header+payload)] [next(header+payload)]
        if (blk->next &&
        ...
        // spajanje sa levim susedom (pomeri fokus ulevo posle spajanja)
        if (blk->prev &&
            ...
            blk = blk->prev; // fokus na veći spojeni blok
            merged = true;
        }
    }
}
