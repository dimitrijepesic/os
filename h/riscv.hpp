...
public:
    ...
    // mask set u sie registru
    static void ms_sie(uint64 mask);

    // mask clear u sie registru
    static void mc_sie(uint64 mask);

    enum BitMaskSie {
        SIE_SEIE = (1 << 9), // Supervisor External Interrupt Enable
        SIE_STIE = (1 << 5), // Supervisor Timer Interrupt Enable
        SIE_SSIE = (1 << 1), // Supervisor Software Interrupt Enable
    };
